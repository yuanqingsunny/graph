...
abstract class MyEdgeRDD[ED](
  ...
  private[graphx] def partitionsRDD: RDD[(PartitionID, MyEdgePartition[ED])]
  ...
  private[graphx] def withTargetStorageLevel(targetStorageLevel: StorageLevel): MyEdgeRDD[ED]
}
...
object MyEdgeRDD {
   ...
   * @tparam ED the edge attribute type
   */
  private[graphx] def fromEdgePartitions[ED: ClassTag]
  (
